...
**Biological example-** compare changes in gene expression between normal (healthy) human lung tissue and idiopathic pulmonary fibrotic (IPF) human lung tissue. 

**Data-** single cell RNA seq data from healthy donor lung tissue and upper IPF lobe tissue (representing early disease progression) and lower IPF lobe tissue (late progression). The data provided in this example are in the most common scRNA-seq file format (matrix/gene/barcode) generated by the CellRanger suite, and the file sizes are a reasonable size for quick analysis.

**Data collection-** single cell RNA seq 3' gene expression comparison; used three healthy donor lungs and separated upper and lower lobes of three IPF lungs. The 3' V2 chemistry kit on Chromium Single cell controller from 10X Genomics was used to generate the libraries. Three additional healthy lung samples were included, with one using the V2 kit, and two using the V1 kit. Brochoalveolar lavage (BAL) samples were also included, and run using the V2 kit. 
...
The nf-core framework for community-curated bioinformatics pipelines. To cite nf-core, please reference the following:

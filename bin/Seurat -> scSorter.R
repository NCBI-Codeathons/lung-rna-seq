...
#sobj1 = GetAssayData(sobj)
#topgene_filter = rowSums(as.matrix(sobj)[topgenes, ]!=0) > ncol(sobj)*.1
#topgenes = topgenes[topgene_filter]

#picked_genes = unique(c(anno$Marker, topgenes))
#expr = expr[rownames(sobj1) %in% picked_genes, ]

rts <- scSorter(sobj1, anno)

#I'm not sure how to combine sobj1 (scSorter output) with sobj (seurat without scSorter) so I made them into separate objects just in case
